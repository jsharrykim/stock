"""
현재 A~F 전략 기준 익절/손절 그리드 백테스트
=========================================

목표:
1) 각 그룹별로 target/stop 조합별 성과 비교
2) 현재 우선순위(A>B>C>D>E>F) 기반 통합 포트폴리오에서
   - 현재 설정
   - 공통 target/stop 설정
   성과 비교

주의:
- 이벤트 데이 필터는 역사 데이터 부재로 미반영
- 나스닥 하락장 필터(A/C/D strict, E/F hysteresis)는 반영
- E/F는 현재 코드와 동일하게 +target 도달 후 MACD 둔화전환 또는 5거래일 대기 만료 매도
"""
...
CURRENT_CONFIG = {
    "A": {"target": 0.15, "stop": 0.25},
    "B": {"target": 0.20, "stop": 0.25},
    "C": {"target": 0.15, "stop": 0.25},
    "D": {"target": 0.15, "stop": 0.25},
    "E": {"target": 0.08, "stop": 0.25},
    "F": {"target": 0.08, "stop": 0.25},
}
...
def signal_by_group(s: dict) -> dict:
    ...
    a = (
        s["above200"]
        and s["golden_cross"]
        and s["pctb_close"] > GOLDEN_CROSS_PCTB_MIN
        and s["rsi"] > GOLDEN_CROSS_RSI_MIN
        and nasdaq_strict
    )
    ...
    b = (
        (not s["above200"])
        and s["vix"] >= VIX_MIN
        and (s["rsi"] < RSI_MAX or s["cci"] < CCI_MIN)
        and s["lr_slope"] > 0
    ...
    c = (
        (not a) and (not b)
        and s["above200"]
        and s["bb_width_prev"] < s["bb_width_avg"] * SQUEEZE_RATIO
        and s["bb_width"] > s["bb_width_prev"] * BB_EXPAND_RATIO
        and s["vol_ratio"] >= SQUEEZE_BREAKOUT_VOL_RATIO
        and s["pctb_close"] > SQUEEZE_BREAKOUT_PCTB_MIN
        and s["macd_hist"] > 0
        and nasdaq_strict
    )
    ...
    d = (
        ...
        and s["adx_rising"]
        and s["macd_hist"] > 0
        and ADX_PCTB_MIN <= s["pctb_close"] <= ADX_PCTB_MAX
        and nasdaq_strict
    )
    ...
    e = (
        ...
        and s["above200"]
        and s["squeeze"]
        and s["pctb_low"] <= SQUEEZE_PCT_B_MAX
        and nasdaq_bottom
    )
    ...
    f = (
        ...
        and s["above200"]
        and s["pctb_low"] <= BB_PCT_B_LOW_MAX
        and nasdaq_bottom
    )
...
def calc_stats(trades: list[dict]) -> dict:
    if not trades:
        return {
            "n": 0, "win_rate": 0.0, "avg_pnl": 0.0, "median_pnl": 0.0,
            "avg_hold": 0.0, "pf": 0.0, "stop_rate": 0.0
        }
...
def main():
    print("=" * 90)
    print("현재 A~F 기준 익절/손절 그리드 백테스트")
    print("=" * 90)
    ...
    for group in GROUPS:
        ...
        for target in TARGET_GRID:
            for stop in STOP_GRID:
                ...
                if trades:
                    group_trade_rows.extend(trades)

    group_df = pd.DataFrame(group_rows).sort_values(["group", "avg_pnl", "pf"], ascending=[True, False, False])
    trades_df = pd.DataFrame(group_trade_rows)

    print("\n[4] 통합 포트폴리오 비교")
    portfolio_rows = []

    current_trades = run_portfolio_backtest(prepared, CURRENT_CONFIG, "current")
    portfolio_rows.append({"scenario": "current", **calc_stats(current_trades)})
    ...
    # 저장
    group_path = os.path.join(os.path.dirname(__file__), "backtest_current_exit_group_grid.csv")
    trades_path = os.path.join(os.path.dirname(__file__), "backtest_current_exit_group_grid_trades.csv")
    portfolio_path = os.path.join(os.path.dirname(__file__), "backtest_current_exit_portfolio_grid.csv")
    ...
    current_by_group = {"A": (15.0, 25.0), "B": (20.0, 25.0), "C": (15.0, 25.0), "D": (15.0, 25.0), "E": (8.0, 25.0), "F": (8.0, 25.0)}
    for group in GROUPS:
